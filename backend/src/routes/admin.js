...
router.get(
  "/overview",
  asyncHandler(async (_req, res) => {
    const [users, sellers, applications, products, regions, pendingProducts] =
      await Promise.all([
        prisma.user.count(),
        prisma.seller.count(),
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Send approval email
    await sendSellerApprovalEmail(application.user, application.id);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    await sendSellerRejectionEmail(updated.user, reason);
